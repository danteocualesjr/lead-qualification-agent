...
app.post('/qualify-lead', async (req, res) => {
  try {
    ...
    const prompt = `Research ${companyName} and provide the following information in JSON format:
    ...
    const response = await axios.post('https://api.perplexity.ai/v1/sonar/query', {
      query: prompt
    }, {
      headers: {
